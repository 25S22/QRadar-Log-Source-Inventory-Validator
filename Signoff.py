...
#
# All devices are expected to be either Windows or Linux.
# If sources are found but no OS signature matches, an amber reply is sent
# listing everything that was found so the team can investigate.
OS_TYPE_GROUPS = {
    'Windows': {
        'required':   ['Microsoft Security Event Log'],
        'companions': {'Microsoft Security Event Log': 'WinCollect'},
    },
    'Linux': {
        'required':   ['Linux OS'],
        'companions': {},
...
def query_all_log_sources_readonly(hostname):
    ...
    Used for every query so OS detection always has the full source list.
...
def validate_expected_types(all_sources_result, required_types, companions):
    """
    Validates a specific set of required types against all returned log sources.

    required_types: list of keyword strings to check (e.g. ['Microsoft Security Event Log'])
...
def detect_os_group(sources):
    ...
    OS is determined purely from the log source types found in QRadar —
    not from anything about the machine running this script.
...
# ─── REPLY HTML BUILDERS ───────────────────────────────────────────────────────
def _build_reply_html_unknown_os(hostname, sources):
    """
    Amber reply used when QRadar returns sources for the hostname but none of
    the log source types match any known OS signature in OS_TYPE_GROUPS.

    Lists every returned source so the team can see exactly what is present
    and investigate whether onboarding is incomplete or misconfigured.
    """
    run_time = datetime.now().strftime('%d %B %Y, %H:%M')

    source_rows = ''
    for src in sources:
        days_val = src.get('days_ago')
        days_str = (
            f"<span style='color:#888;font-size:11px;'>"
            f"({'Today' if days_val == 0 else str(days_val) + ' days ago'})"
            f"</span>"
            if days_val is not None
            else "<span style='color:#aaa;font-size:11px;'>(no events yet)</span>"
        )
        enabled_badge = (
            "<span style='color:#1a7a4a;font-weight:600;'>Enabled</span>"
            if src.get('enabled')
            else "<span style='color:#c0392b;font-weight:600;'>Disabled</span>"
        )
        source_rows += f"""
        <tr>
          ...
          <td style="padding:8px 12px;border-bottom:1px solid #e8e8e8;
            ...
            {src.get('ls_type', 'N/A')}</td>
          <td style="padding:8px 12px;border-bottom:1px solid #e8e8e8;
                     font-size:12px;color:#555;">
            {src.get('last_seen', 'N/A')} {days_str}</td>
          <td style="padding:8px 12px;border-bottom:1px solid #e8e8e8;
                     font-size:12px;">{enabled_badge}</td>
        </tr>"""

    onboard_note = (
        f"<p style='margin:12px 0 0 0;font-size:12px;color:#7a5500;'>"
        f"{ONBOARD_REQUEST_NAME} — please review the log source types above and "
        f"confirm whether the correct Windows (<b>Microsoft Security Event Log</b>) or "
        f"Linux (<b>Linux OS</b>) source type has been onboarded.</p>"
...
<body style="font-family:'Segoe UI',Arial,sans-serif;color:#222;
  ...
  <div style="max-width:660px;padding:20px 0;">
    ...
    <div style="background:#c87800;color:#fff;padding:10px 16px;
                border-radius:6px;font-size:13px;font-weight:600;
                margin-bottom:16px;letter-spacing:0.2px;">
      ⚠&nbsp; Found in QRadar — Unrecognized Log Source Type
    </div>
    <p style="margin:0 0 4px 0;">
      <b>{hostname}</b> was found in QRadar but the log source type(s) present
      do not match any known OS signature
      (<b>Linux OS</b> for Linux, <b>Microsoft Security Event Log</b> for Windows).
      The sources found are listed below — please investigate.
    </p>
    <table style="width:100%;max-width:660px;border-collapse:collapse;
...
def _build_reply_html(hostname, qradar_result, type_validation=None, os_group=None):
    ...
    States:
      Not Found / API Error         → red banner
      All types found, all
...
def build_reply_body(hostname, qradar_result):
    """
    Routes to the correct HTML builder based on the QRadar result.

    Not Found / API Error:
      Red banner — hostname not in QRadar at all.

    Found, OS signature matched (Windows / Linux):
      Validates required types and companion rules for that OS group only.
      Green if everything present, amber if anything missing.

    Found, no OS signature matched:
      Amber 'Unrecognized Log Source Type' reply listing all sources found.
      All environments are expected to be Windows or Linux — anything else
      warrants investigation.

    Returns (html_body, needs_cc) tuple.
    needs_cc is True when any type/companion is missing or OS is unrecognized.
    """
    # Not found or API error — no CC needed
    ...
    # Sources found but no OS signature matched — amber unknown-type reply
    if group_name is None:
        html = _build_reply_html_unknown_os(hostname, sources)
        return html, bool(ONBOARD_REQUEST_CC.strip())

    # Known OS — validate its required types and companions
...
def main():
    ...
    try:
        ...
        for mail_item in inbox_items:
            ...
            # ── QRadar query — always fetch all sources for OS detection ──
